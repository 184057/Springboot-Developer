package me.jeong.SpringbootDeveloper.config.jwt;
...
// 1. 토큰을 생성하는 메서드로, 인자는 만료 시간, 유저 정보를 받는다
//    이 메서드에서는 set 계열의 메서드를 통해 여러 값을 지정한다.
//    헤더는 typ(타입), 내용은 iss(발급자), iat(발급일시), exp(만료일시), sub(토큰제목)이, 클레임에는 유저ID를 지정
//    토큰을 만들 때는 프로퍼티즈 파일에 선언해둔 비밀값과 함께 HS256방식으로 암호화한다.

// 2. 토큰이 유효한지 검증하는 메서드
//    프로퍼티즈 파일에 선언한 비밀값과 함께 토큰 복호화를 진행한다.
//    복호화 과정에서 에러가 발생하면 유효하지 않은 토큰이므로 false, 아무 에러도 발생하지 않으면 true를 반환한다

// 3. 토큰을 받아 인증 정보를 담은 객체 Authentication를 반환하는 메서드
//    프로퍼티즈 파일에 저장한 비밀 값으로 토큰을 복호화한 뒤 클레임을 가져오는 private 메서드인 getClaims()를 호출해서
//    클레임 정보를 반환받아 사용자 이메일이 들어 있는 토큰 제목 sub와 토큰 기반으로 인증정보를 생성한다.
//    이때 UsernamePasswordAuthenticationToken의 첫 인자로 들어가는
//    User는 프로젝트에서 만든 User클래스가 아닌, 스프링 시큐리티에서 제공하는 User클래스를 임포트해야한다.

// 4. 토큰 기반으로 사용자 ID를 가져오는 메서드
//    프로퍼티즈 파일에 저장한 비밀값으로 토큰을 복호화한 다음 클레임을 가져오는 private 메서드인 getClaims()를 호출해서
//    클레임 정보를 반환받고 클레임에서 id키로 저장된 값을 가져와 반환한다.
