...
class TokenProviderTest {
    ...
    void generateToken() {
        ...
        // then : jjwt 라이브러를 사용해 토큰을 복호화한다. 토큰을 만들 때 클레임으로 넣어둔 id 값이
    ...
    void validToken_invalidToken() {
        // given : jjwt 라이브러리를 사용해 토큰을 생성한다. 이때 만료시간은 1970년 1월 1일로부터
        //         현재 시간을 밀리초단위로 치환한 값(new Date.getTime())에 1000을 빼, 이미 만료된 토큰으로 생성
    ...
    void validToken_validToken() {
        // given : jjwt 라이브러리를 사용해 토큰을 생성한다. 만료 시간은 현재시간으로부터 14일 뒤로,
        //         만료되지 않은 토큰으로 생성한다.
    ...
    void getAuthentication() {
        ...
        // then : 반환받은 인증 객체의 유저 이름을 가져와
        //        given절에서 설정한 subject값인 "user@email.com"과 같은지 확인한다
    ...
    void getUserId() {
        // given : jjwt 라이브러리를 사용해 토큰을 생성한다.
        //         이때 클레임을 추가한다. 키는 "id", 값은 1이라는 유저ID이다.
        Long userId = 1L;
        String token = JwtFactory.builder()
